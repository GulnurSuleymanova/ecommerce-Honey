...
const Header = () => {
  ...
  className="hidden md:flex group bg-white text-gray-700 duration-300 h-[45px] w-[45px] md:h-[50px] md:w-[50px] items-center justify-center rounded-full hover:bg-amber-950 cursor-pointer"
>
  <Globe size={20} className="group-hover:text-white duration-300" />
              ...
              {openLang && (
                <div className="absolute top-12 left-0 bg-white border rounded-lg shadow-lg p-2 w-28 text-amber-300">
                  <button
                    onClick={() => changeLang("az")}
                    className={`block w-full text-left px-3 py-1 rounded ${i18n.language === "az" ? "bg-amber-950 text-amber-300" : "hover:bg-gray-100"}`}
                  >
                    AZ
                  </button>
                  <button
                    onClick={() => changeLang("en")}
                    className={`block w-full text-left px-3 py-1 rounded ${i18n.language === "en" ? "bg-amber-950 text-amber-300" : "hover:bg-gray-100"}`}
                  >
                    EN
                  </button>
                </div>
          ...
          <div className="hidden md:flex items-center gap-2 z-40">
            ...
            <div className="relative flex items-center gap-2">
              {user ? (
                <>
                  <span
                    className="font-medium tracking-wide text-gray-700 cursor-pointer"
                  ...
                  {openUser && (
                    <div className="absolute top-12 right-0 bg-white border rounded-lg shadow-lg p-2 w-32">
                      <button
                        onClick={() => { logoutUser(); setOpenUser(false); }}
                  ...
                  {openUser && (
                    <div className="absolute top-12 right-0 bg-white border rounded-lg shadow-lg p-2 w-32">
                      <button
                        ...
                        className="block w-full text-left px-3 py-1 rounded hover:bg-gray-100 text-black"
                      >
                        Login
                      </button>
                      <button
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white shadow-[0_-8px_29px_0_rgba(0,0,0,0.1)] grid grid-cols-5 md:hidden text-center text-[#9b9b9b] z-50">
        ...
        {user ? (
          <span className="py-2 text-xs">{user.firstName}</span>
        ) : (
      ...
      <div className={`fixed top-0 left-0 h-full w-64 bg-[#f6c35c] shadow-lg z-100 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
